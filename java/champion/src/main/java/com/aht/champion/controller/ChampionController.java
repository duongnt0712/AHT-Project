...
import com.aht.champion.repository.model.*;
...
@Controller
public class ChampionController<T> {
    ...
    @PostMapping("/pos")
    public ResponseEntity<Object> push(@RequestBody JSONObject objects) {
//        System.out.println(((Student) hashMapToModel("student", objects.get(0))).toString());
//        System.out.println(((Product) hashMapToModel("product", objects.get(1))).toString());

        Gson gson = new GsonBuilder().create();
        Student s = gson.fromJson(String.valueOf(objects.get("student")), Student.class);
        System.out.println(s.toString());
        return new ResponseEntity<>(HttpStatus.OK);
    }

    private T hashMapToModel(String t, LinkedHashMap l) {
        if (t.equalsIgnoreCase("student")) {
            Student s = new Student();
            s.setId((Integer) l.get("id"));
            s.setName((String) l.get("name"));
            return (T) s;
        } else {
